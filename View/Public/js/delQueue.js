...
function confirmDel() {
    ...
    custId = this.parentNode.parentNode.children[2].innerHTML;
    // console.log("上桌顾客的排队号：" + custId);
...
function confirmTop() {
    ...
    var Global_custId = this.parentNode.parentNode.children[2].innerHTML;
    console.log("上桌顾客的排队号：" + Global_custId);
    //console.log(id);
    var index = $(this).parent().parent().index();
    //console.log(index);
    var table = this.parentNode.parentNode.parentNode.parentNode;
...
function confirmSendMessage() {
    ...
    var thisNode = this;
    var custQueneNum = thisNode.parentNode.parentNode.children[2].innerHTML;
...
function refreshWaitCustTotal(cirNum) {
    ...
    if (is_empty == "emptyTip") {
        ...
        var trNum = document.getElementById(tabTableId).getElementsByTagName('tr').length;
    }
    ...
    var lastCirCurrentShowNum = lastCir.innerHTML.slice(-1);
